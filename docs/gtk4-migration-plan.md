...
| `src/server.c`, `src/stat.c` | Server response packet parsing; address/port parsing — `server.c` also pulls in `game.c`'s generated games database and `stat.c` pulls in `dialogs.h`/`source.h`, more entangled than a quick add |
