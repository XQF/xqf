			...
			if ( content[i-1] != '\n') {
				...
				do {
					...
					*(png_start+4)=0; /*cut anything behind ".png" */
					...
					png_filename= g_strconcat(PACKAGE_DATA_DIR, G_DIR_SEPARATOR_S,"default", G_DIR_SEPARATOR_S, "trayicon", G_DIR_SEPARATOR_S, line, NULL);
...
void tray_create_menu (void)
{
	...
	g_signal_connect(G_OBJECT(refresh_item), "activate", G_CALLBACK(refresh_n_server), GINT_TO_POINTER(N_SERVER));
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), refresh_item);

	update_item= gtk_menu_item_new_with_label(_("Update"));
	g_signal_connect(G_OBJECT(update_item), "activate", G_CALLBACK(update_source_callback), NULL);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), update_item);

	stop_item = gtk_menu_item_new_with_label(_("Stop"));
	g_signal_connect(G_OBJECT(stop_item), "activate", G_CALLBACK(stop_callback), NULL);
	...
	g_signal_connect(G_OBJECT(show_item), "activate", G_CALLBACK(show_main_window_call), NULL);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), show_item);

	// translator: hide main window
	hide_item= gtk_menu_item_new_with_label(_("Hide"));
	g_signal_connect(G_OBJECT(hide_item), "activate", G_CALLBACK(hide_main_window_call), NULL);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), hide_item);

	exit_item = gtk_menu_item_new_with_label(_("Exit"));
	g_signal_connect(G_OBJECT(exit_item), "activate", G_CALLBACK(exit_call), NULL);
...
gboolean kde_dock (EggTrayIcon *icon)
{
	...
	data = 1;
	XChangeProperty (GDK_DISPLAY(), win, kde_dock_atom, XA_WINDOW, 32, PropModeReplace, (unsigned char *)&data, 1);
	data = GDK_WINDOW_XID (window->window);
	XChangeProperty (GDK_DISPLAY(), win, kde_tray_atom, XA_WINDOW, 32, PropModeReplace, (unsigned char *)&data, 1);
